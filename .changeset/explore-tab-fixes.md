...
- **List-level viewport tracker on `PageFeed`** (`src/pages/PageFeed/PageFeed.renderer.tsx` + new `src/viewable/ViewableContext.tsx`) — `FlatList.onViewableItemsChanged` with a 50% threshold + 100 ms minimum-view-time dispatches `on_view` exactly once per top-level item via a `firedViewIdsRef` Set. `ViewableContext` is scaffolded so future surfaces can suppress nested `Viewable.onLayout` fallbacks without breaking the current per-card `on_view` path.
