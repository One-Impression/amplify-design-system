...
- [x] Phase 4 — fixtures. Captured real wire JSON from the local gateway: `campaign-card` (creator.snippet.card) + 3 explore sections. Registry in `src/fixtures/index.ts`.
...
- [ ] All-snippets coverage — currently 5 of ~43 snippets have authored samples (`card, info_row, section_header, group_chips, separator`). Expand `samples.ts` to cover every `creator.snippet.*` (read each `*.schema.js` for the data shape). Same for more bottom-sheet variants (bottom_sheet_header/footer/input) and the `web_view` page.
