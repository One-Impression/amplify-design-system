...
## tokens-brand 3.1.0 — 2026-05-08

### Added (additive — `@amplify-ai/tokens-brand`)

Closes [#88](https://github.com/One-Impression/amplify-design-system/issues/88) — ship a Tailwind v4 consumer preset CSS so Canvas's semantic class names (`bg-brand`, `bg-surface`, `text-primary`, `bg-positive`, etc.) resolve to brand-tier values.

- **New export `./preset`** — `@amplify-ai/tokens-brand/preset` ships `dist/preset.css`, a self-contained Tailwind v4 preset that extends the existing `tailwind.css` `@theme` block with consumer aliases mapping Canvas's class-name vocabulary (`bg-brand`, `bg-accent`, `bg-surface`, `text-primary`, `text-positive`, `border-border-default`, etc.) onto the appropriate `theme-color-*` and `semantic-*` tier tokens already defined by this package.
- **Convenience direct path** — `./dist/preset.css` also exposed for consumers who hardcode the dist path (mirrors the `./tailwind` pattern from PR #87).
- **No breaking changes** — `tailwind.css` and all other exports are unchanged. Existing consumers keep working; new consumers can opt into `/preset` to skip the inline `@theme` bridge.
...
The alias mapping is provisional pending review — see PR description for the four open design questions surfaced from Issue #88's body (canonical class-name set, per-product preset behaviour, hand-written vs generated, which token tier feeds the preset). The current implementation is generated by `scripts/build-tokens.js` so it tracks the source-of-truth tokens and won't drift.
