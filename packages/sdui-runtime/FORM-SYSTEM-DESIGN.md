...
## 11. Beyond the form — playground layout/chrome shipped alongside

These landed while polishing the playground; they aren't form-specific but share
the same "wire stays content, runtime/tokens own device chrome" discipline.
...
to the same form field, `on_change: dismiss`. No frontend special-casing, no
`data/countries.ts` runtime dataset — the option list ships in the wire. (The old
`data/countries.ts` + `useCountryPickerChip` + bespoke `PhoneNumberInput` are dead,
flagged for deletion in §8.)

**`group_config` is the universal layout primitive.** Recursive container reading
`stacking` (horizontal/vertical) + raw `{gap, align, justify, item_flex, card}`.
`item_flex:"equal"` wraps each child in `<Box flex={1}>` so N cards fill the row
evenly; `card:true` wraps the whole group in a `DSCard`. Friendly align/justify
values map to RN. This replaces per-snippet stacking hacks.

**Page chrome is runtime-owned (device values + design tokens), wire stays content:**
- **Bottom safe-area + buffer** — every scroll page applies
  `paddingBottom: insets.bottom + sdui.spacing.lg` (`useSafeAreaInsets`), so the
  last item clears the home indicator on all page types. No BFF input needed.
- **Header SLOT — symmetric with the footer slot.** `data.header` on any page
  (`page_standard`, `page_sticky_footer`, `page_feed`) and on any `bottom_sheets[]`
  entry takes a `page_header` snippet. When present, the runtime **hides the native
  nav header** (`SduiNavigationHost` page screen `setOptions({headerShown:false})`)
  and the wire snippet owns the top chrome:
  - top safe-area inset (`useSafeAreaInsets`) clears the status bar;
  - **background** read raw from `node.data.background` (wire extension, not yet in
    schema): a color string → solid fill, or `{gradient:{colors,angle?}}` → gradient
    via the existing `Gradient` component (fills under the status bar too);
  - `left_icon` is the back affordance — wrapped in a `Pressable` → `goBack()`;
  - icons render through the icon-store glyph path (`useIconStore().getIcon` →
    `parseSvg`), the same pattern `IconRenderer` uses — a bare `<DSIcon name=…/>`
    only draws an empty box.
  In the sheet host the header renders **outside** `BottomSheetScrollView` (pinned,
  like the footer), replacing the plain `sheet.title` text when present.

  Demo: `demo.form` carries a gradient `page_header` (`["#7C3AED","#5B21B6"]`, angle
  120) with a white title/subtitle + `back-icon`.

  > Playground note: content icons currently paint the icon-store **fallback box**
  > (the manifest serves real SVGs at `/v1/creator/assets/icons-manifest`, but the
  > store isn't populating in-session — pre-existing, affects all `IconRenderer`
  > glyphs, not introduced by the header slot).
